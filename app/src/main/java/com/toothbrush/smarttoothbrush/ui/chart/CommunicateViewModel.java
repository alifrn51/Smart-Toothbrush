...
import com.toothbrush.smarttoothbrush.di.TimerApp;
...
public class CommunicateViewModel extends AndroidViewModel {
    ...
    public void startTimer() {
        TimerApp timerObj = new TimerApp();
        Observable.interval(1, TimeUnit.SECONDS)
                .observeOn(Schedulers.io())
                .takeWhile(aLong -> isStart)
                .subscribe(new Observer<Long>() {
                    ...
                    @Override
                    public void onNext(@NotNull Long seconds) {


                        if (!timerObj.getHours().equals("59")) {
                            if (!timerObj.getMinutes().equals("59")) {
                                if (!timerObj.getSeconds().equals("59")) {
                                    int sec = Integer.parseInt(timerObj.getSeconds()) + 1;
                                    timerObj.setSeconds(String.valueOf(sec < 10 ? "0" + sec : sec));
                                } else {
                                    timerObj.setSeconds("00");
                                    int sec = Integer.parseInt(timerObj.getMinutes()) + 1;
                                    timerObj.setMinutes(String.valueOf(sec < 10 ? "0" + sec : sec));
                                }
                            } else {
                                timerObj.setMinutes("00");
                                int min = Integer.parseInt(timerObj.getHours()) + 1;
                                timerObj.setHours(String.valueOf(min < 10 ? "0" + min : min));
                            }
                        }
                        timerLiveData.postValue(timerObj);
    ...
    //Getter timer
    public LiveData<TimerApp> getTimerLiveData() {
